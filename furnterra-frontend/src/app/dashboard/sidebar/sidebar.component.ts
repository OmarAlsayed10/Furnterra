...
export class SidebarComponent implements OnInit {
  ...
  activeIndex: { section: 'main' | 'settings'; index: number } = {
    section: 'main',
    index: 0,
  };
    ...
    this.mainIcons = [
      {
        ...
        svg: this.Sanitizer
          .bypassSecurityTrustHtml(`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#000000"
  ...
  activeIconRoute(url: string) {
    const mainMatch = this.mainIcons.findIndex(
      (icon) => `/dashboard/${icon.route.split('/').pop()}` === url
    );
    const settingsMatch = this.settingsIcons.findIndex(
      (icon) => `/dashboard/${icon.route.split('/').pop()}` === url
    );
    if (mainMatch !== -1) {
  ...
  iconActive(section: 'main' | 'settings', index: number) {
    ...
    const route =
      section === 'main'
        ? this.mainIcons[index].route
        : this.settingsIcons[index].route;
